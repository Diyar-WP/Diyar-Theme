...
function diyar_themes_sync_data_arab_to_english($post_id, $post, $update)
{
    session_start();
...
add_action('save_post', 'diyar_themes_sync_data_arab_to_english', 10, 3); /// end simple
...
function arab_validate_country_post_title($data, $postarr, $update)
{
    ...
    if ($postarr['ID'] === 0 && isset($postarr['post_type']) && $postarr['post_type'] === 'countries') {
        ...
        if (!empty($existing_post_ids)) {
            ...
            $error_message = __('A post with the same title already exists. Please choose a different title.', 'diyar_theme');
